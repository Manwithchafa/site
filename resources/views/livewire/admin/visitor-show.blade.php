    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <x-admin.card class="p-6 xl:col-span-1">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-semibold text-white">{{ $visitor->full_name }}</h2>
                    <p class="text-sm text-slate-500">{{ ucfirst($visitor->sex) }} · {{ $visitor->phone }}</p>
                </div>
            </div>

            <dl class="mt-6 space-y-4 text-sm">
                <div><dt class="text-slate-500">Email</dt><dd class="mt-1 font-medium text-white">{{ $visitor->email ?: 'Not provided' }}</dd></div>
                <div><dt class="text-slate-500">Residential Address</dt><dd class="mt-1 font-medium text-white">{{ $visitor->residential_address ?: 'Not provided' }}</dd></div>
                <div><dt class="text-slate-500">Nearest Bus Stop</dt><dd class="mt-1 font-medium text-white">{{ $visitor->nearest_bus_stop ?: 'Not provided' }}</dd></div>
                <div><dt class="text-slate-500">Occupation</dt><dd class="mt-1 font-medium text-white">{{ $visitor->occupation ?: 'Not provided' }}</dd></div>
                <div><dt class="text-slate-500">Invited By</dt><dd class="mt-1 font-medium text-white">{{ $visitor->invited_by ?: 'Not provided' }}</dd></div>
            </dl>
            ...
            <div class="mt-6 space-y-5">
                @forelse ($timeline as $item)
                    <div class="relative flex gap-4">
                        <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-xs font-semibold text-slate-300">
                            {{ strtoupper(substr($item['type'], 0, 1)) }}
                        </div>
                        <div class="min-w-0 flex-1 rounded-2xl border border-white/10 p-4">
                            <div class="flex flex-wrap items-start justify-between gap-2">
                                <p class="font-semibold text-white">{{ $item['title'] }}</p>
                                <span class="text-xs text-slate-500">{{ $item['timestamp']->format('M j, Y · g:i A') }}</span>
                            </div>
                ...
                @forelse ($visitor->assignments->sortByDesc('assigned_at') as $assignment)
                    <div class="rounded-2xl border border-white/10 p-4">
                        <div class="flex items-center justify-between gap-3">
                            <p class="text-sm font-semibold text-white">{{ $assignment->assignee?->name ?? 'Unassigned' }}</p>
        ...
        <x-admin.card class="p-6">
            ...
            <div class="mt-5 space-y-3">
                @forelse ($visitor->notes->sortByDesc('created_at') as $note)
                    <div class="rounded-2xl border border-white/10 p-4">
                        <div class="flex items-center justify-between gap-3">
                            <p class="text-sm font-semibold text-white">{{ ucfirst($note->type) }}</p>
                            <span class="text-xs text-slate-500">{{ $note->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="mt-2 text-sm leading-6 text-slate-300">{{ $note->body }}</p>
                    </div>
                @empty
        ...
        <x-admin.card class="p-6">
            ...
            <div class="mt-5 space-y-3">
                @forelse ($visitor->registrations->whereNotNull('prayer_request')->where('prayer_request', '!=', '') as $registration)
                    <div class="rounded-2xl border border-white/10 p-4">
                        <p class="text-sm leading-6 text-slate-300">{{ $registration->prayer_request }}</p>
                        <p class="mt-2 text-xs text-slate-500">{{ $registration->created_at->format('M j, Y') }}</p>
                    </div>
                @empty
